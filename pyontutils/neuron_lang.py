...
def setLocalNameTrip(LocalName, phenoId, predicate):
    setLocalName(LocalName, Phenotype(phenoId, predicate))

def setLocalName(LocalName, phenotype):
    setLocalNameBase(LocalName, phenotype, globals())
...
def main():
    ...
    [setLocalNameTrip(*_) for _ in bbp_names]
    setLocalName('U', NegPhenotype(Tu))  # annoying to have to wait for these... but ok
    setLocalName('L23', LogicalPhenotype(OR, L2, L3))
    setLocalName('ASp', NegPhenotype(Sp))
