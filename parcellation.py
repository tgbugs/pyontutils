...
sgv = Vocabulary(cache=True, verbose=True)
...
ABAROOT = namedtuple('ABAROOT', ['id', 'name', 'safe_name', 'acronym','prefix'])
...
def aba_base(new_graph, root, superclass):
    ...
    def aba_trips(node_d):
        output = []
        parent = root.prefix + ':' + str(node_d['id'])  # FIXME HRM what happens if we want to change ABA:  OH LOOK
    ...
    for node_d in resp['msg']:
        if node_d['id'] == root.id:  # FIXME need a better place to document this :/
...
def human_brain_atlas():
    ...
    new_graph = makeGraph(filename, PREFIXES)
    new_graph.add_node(ontid, rdflib.RDF.type, rdflib.OWL.Ontology)
    new_graph.add_node(ontid, rdflib.RDFS.label, 'Allen Human Brain Atlas Ontology')  # FIXME should come from atlas.name?
    new_graph.add_node(ontid, rdflib.RDFS.comment, 'This file is automatically generated from the Allen Brain Atlas API')
    new_graph.add_node(ontid, rdflib.OWL.versionInfo, TODAY)
