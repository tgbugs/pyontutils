...
class makeGenericPScheme:
    ...
    def __init__(self):
        ontid = self.ont.path + self.ont.filename + '.ttl'
        self.PREFIXES.update(makeGenericPScheme.PREFIXES)
        graph = makeGraph(self.ont.filename, self.PREFIXES)
        graph.add_ont(ontid, *self.ont[2:])
        make_scheme(graph, self.concept, self.atlas.curie)
        data = self.datagetter(self.ont, self.concept, self.atlas)
        self.dataproc(graph, data, self.ont, self.concept)
        add_ops(graph)
        graph.write(delay=True)

    @staticmethod
    def datagetter(ont, concept, atlas):
    ...
    @staticmethod
    def dataproc(graph, data, concept):
        """ example datagetter function, make any local modifications here """
        for thing in data:
            graph.add_node(*thing)
        raise NotImplementedError('You need to implement this yourlself!')

class MBA(makeGenericPScheme):
    ...
    @staticmethod
    def datagetter(*args):
        url = 'http://api.brain-map.org/api/v2/tree_search/Structure/{root}.json?descendants=true'.format(root=MBA.ROOT)
        resp = requests.get(url).json()
        for node in resp['msg']:
            if node['id'] == MBA.ROOT:
    ...
    @staticmethod
    def dataproc(graph, data, ont, concept):
        for node in data:
            curie = graph.expand('ABA:' + str(node['id']))
            graph.add_class(curie, concept.curie)
            parent = node['parent_structure_id']
            graph.add_node(curie, rdflib.RDFS.label, '(%s) ' % ont.shortname + node['name'])
...
class HBA(MBA):
    ...
    @staticmethod
    def datagetter(ont, concept, atlas):
        url = 'http://api.brain-map.org/api/v2/tree_search/Structure/{root}.json?descendants=true'.format(root=HBA.ROOT)
        resp = requests.get(url).json()
        return resp['msg']
