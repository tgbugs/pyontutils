...
: ${ZIP_LOCATION:=<<build-location-graph()>>}
: ${GIT_LOCATION:=<<build-location-graph()>>}
: ${GIT_REF:="master"}
...
function compile-scigraph-git () {
    ...
    ontload scigraph \
    --zip-location ${ZIP_LOCATION}  ${IFS# build artifacts will be deposited here} \
    --git-local ${GIT_LOCATION}     ${IFS# remote repos will be cloned here} \
...
: ${BUILD_LOCATION:=<<build-location-graph()>>}
...
function deploy-graph () {
    ...
    if [ "${SERVER}" = "localhost" ]; then
        ...
        ssh ${scigraph_host} "unlink \"${RUNTIME_LOCATION}/${FOLDER_NAME_RUNTIME}\"
                              ln -sT \"${RUNTIME_LOCATION}/${STAMPED}\" \"${RUNTIME_LOCATION}/${FOLDER_NAME_RUNTIME}\""
...
: ${BUILD_LOCATION:=<<build-location-services()>>}
: ${RUNTIME_LOCATION:=<<runtime-location()>>}
: ${FOLDER_NAME_RUNTIME:=<<folder-name-runtime()>>}
...
SERVICES_GRAPH_PATH="${RUNTIME_LOCATION}/${FOLDER_NAME_GRAPH}"
YAML="${FILE_STEM}.yaml"
DT=$(date +%Y%m%dT%H%M%S)
YAML_RAW="${FILE_STEM}-${DT}.yaml.raw"

PATH_YAML="${BUILD_LOCATION}/${YAML}"
PATH_YAML_RAW="${BUILD_LOCATION}/${YAML_RAW}"
#+end_src
...
: ${CONFIG:=<<build-location-services()>>/<<file-stem-services-output()>>.yaml}
: ${SERVER:=localhost}
: ${RUNTIME_LOCATION:=<<runtime-location()>>}

CONFIG_RAW=$(head -n 1 "${CONFIG}" | cut -b2-)
YAML=$(basename "${CONFIG}")
EXISTING="${RUNTIME_LOCATION}/${YAML}"

scigraph_host="${SERVER}-deploy"  # by convention
scigraph_host_admin=${SERVER}
if [ ${SERVER} = localhost ]; then
    local TARGET="${RUNTIME_LOCATION}"
else
    local TARGET="${scigraph_host}:${RUNTIME_LOCATION}"
fi
#+end_src
...
#+begin_src bash
YAML_RAW=$(head -n 1 "'${EXISTING}'" | grep "^#" | cut -b2-)  # on remote
if [ -n "${YAML_RAW}" ]; then
    YAML_BACKUP="'${RUNTIME_LOCATION}'/${YAML_RAW//.raw/}"
else
    mod_dt=$(stat -c %y "'${EXISTING}'" | cut -d "." -f1 | sed "s/[-\:]//g" | sed "s/\ /T/")  # on remote
    YAML_BACKUP="'${RUNTIME_LOCATION}/${YAML}'-${mod_dt}"
fi
...
function deploy-services () {
    ...
    if [ ${SERVER} = localhost ]; then
        ...
        echo ${CMD}
        #cp "${CONFIG_RAW}" "${TARGET}"
        #cp "${CONFIG}" "${TARGET}"
    else
        echo "${CMD}"
        ssh ${scigraph_host} "${CMD}"
        #scp "${CONFIG_RAW}" "${TARGET}"  # if only we could rsync
        #scp "${CONFIG}" "${TARGET}"
        #ssh ${scigraph_host_admin} "sudo systemctl start scigraph"
    fi
}
...
#+header: :shebang "#!/usr/bin/env bash" :tangle-mode (identity #o755)
#+header: :var PATH_ONTOLOGIES=path-ontologies-nifstd()
#+header: :var BUILD_LOCATION=build-location-nifstd()
#+header: :var GIT_REF=git-ref-nifstd()
...
#+header: :shebang "#!/usr/bin/env bash" :tangle-mode (identity #o755)
#+begin_src bash :noweb yes
./bin/build-services --path-cypher-resources <<cypher-resources-nifstd()>>
#+end_src
**** deploy
** sparc
*** data
...
#+header: :shebang "#!/usr/bin/env bash" :tangle-mode (identity #o755)
#+header: :var PATH_ONTOLOGIES=path-ontologies-sparc()
#+header: :var BUILD_LOCATION=build-location-sparc()
#+header: :var GIT_REF=git-ref-sparc()
...
<<*source-scigraph-functions>>
deploy-services --server aws-scigraph
#+end_src
** sparc-data
*** data
apinatomy build
*** graph
**** load
Set =build-location-graph-sparc-data= here
...
#+header: :noweb yes :eval never :results output
#+header: :shebang "#!/usr/bin/env bash" :tangle-mode (identity #o755)
#+header: :var BUILD_LOCATION=build-location-sparc-data()
#+header: :var FOLDER_NAME_GRAPH=folder-name-graph-sparc-data()
#+header: :var PATH_ONTOLOGIES=path-ontologies-sparc-data()
...
<<*source-scigraph-functions>>
deploy-services --server aws-scigraph-data
#+end_src
**** build-deploy
** dev local ontology
*** graph
**** deploy
#+begin_src bash :noweb yes
./bin/deploy-graph --build-location
#+end_src
*** services config
**** build
**** deploy
...
<<*source-scigraph-functions>>
deploy-graph \
--build-location <<build-location-sparc-data()>> \
...
<<*source-scigraph-functions>>
deploy-services --config <<build-location-services()>>/<<file-stem-sparc-data-local()>>.yaml
#+end_src
**** build-deploy
