...
curl https://cassava.ucsd.edu/sparc/preview/exports/curation-export.ttl -o <<&path-build-sparc-data()>>/curation-export.ttl
...
curl https://cassava.ucsd.edu/sparc/preview/exports/curation-export.ttl -o <<&path-build-sparc-data-dev()>>/curation-export.ttl
...
: services-sparc
** dev local sparc-sckan
*** deploy
#+name: run-deploy-graph-sparc-sckan-local
#+header: :shebang "#!/usr/bin/env bash" :tangle-mode (identity #o755)
#+begin_src bash :noweb yes :tangle ./bin/run-deploy-graph-sparc-sckan-local
<<&source-scigraph-functions>>
deploy-graph \
--path-build <<&path-build-sparc-sckan()>>
#+end_src
...
function load-graph () {
    ...
    mv ${FOLDER_NAME_GRAPH} ${STAMPED} || return $?
