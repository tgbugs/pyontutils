...
** sparc-sckan
# FIXME it is going to be much easier to implement the sckan build in
# release.org instead of doing it here
*** graph
**** load
:properties:
:visibility: folded
...
# FIXME need to source ontologies from the git repo in a sane way and share with the blazegraph load
...
sed 's,sparc-data.ttl,file://<<&path-build-sparc-sckan()>>/sparc-sckan.ttl,g' | \
sed 's,http://ontology.neuinfo.org/NIF/,https://raw.githubusercontent.com/SciCrunch/NIF-Ontology/dev/,g' > \
<<&path-build-sparc-sckan()>>/sparc-sckan.ttl

# FIXME source these from kg release
pushd ~/git/apinatomy-models/models/
    for model_id in $(ls -d *); do
        cp ${model_id}/derived/${model_id}.ttl <<&path-build-sparc-sckan()>>/;
    done
popd

# TODO construct the published only version, probably from individual
# ttls or from the materialized release
curl https://cassava.ucsd.edu/sparc/preview/exports/curation-export.ttl | \
     ${PY_X} -m ttlser.ttlfmt -f ttl -t xml -o <<&path-build-sparc-sckan()>>/curation-export.owl

# FIXME we need some of the ontload functionality for this, at the moment it is too rigid
load-graph \
