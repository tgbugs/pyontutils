...
PY_X=$(command -v pypy3 1>& 2 || command -v python)

mkdir <<&path-build-sparc-data-dev()>>
...
for model_id in {bolser-lewis,bronchomotor,ard-arm-cardiac,sawg-distal-colon,vagus-nerve,scaffold-test}; do
    cp ~/git/apinatomy-models/${model_id}/derived/${model_id}.ttl <<&path-build-sparc-data-dev()>>/;
done
