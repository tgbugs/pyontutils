...
case $key in
    ...
    --zip-path)            local ZIPPATH="${2}"; shift; shift ;;
    *) POSITIONAL+=("$1"); shift ;;
esac
done
...
: ${ZIPPATH="${BUILD_LOCATION}/${ZIPNAME}"}
if [ ! -f "${ZIPPATH}" ]; then
    echo $ZIPPATH path does not exist.
    echo Not deploying.
    return 1
fi
...
function deploy-graph () {
    <<*vars-deploy-graph>>
    echo Will deploy graph to ${SERVER}:${RUNTIME_LOCATION}
    echo $ZIPPATH
    <<*are-you-sure>>
...
if [ -n "${E_YAML_RAW}" ]; then
    YAML_BACKUP="'${RUNTIME_LOCATION}'/${E_YAML_RAW//.raw/}"
else
...
<<*source-scigraph-functions>>
load-graph-ontload \
--build-location <<build-location-nifstd()>> \
...
<<*source-scigraph-functions>>
load-graph-ontload \
--build-location <<build-location-sparc()>> \
--folder-name-graph <<folder-name-graph()>> \
...
<<*source-scigraph-functions>>
deploy-services \
--config <<build-location-sparc()>>/<<file-stem-services-output()>>.yaml \
--server aws-scigraph
#+end_src
** sparc-data
*** data
...
*** graph
**** load
Set =build-location-sparc-data= here
...
<<*source-scigraph-functions>>
deploy-services \
--config <<build-location-sparc-data()>>/<<file-stem-services-output()>>.yaml \
--server aws-scigraph-data
#+end_src
**** build-deploy
** dev local ontology
...
<<*source-scigraph-functions>>
deploy-services --config <<build-location-sparc()>>/<<file-stem-services-output()>>.yaml
#+end_src
**** build-deploy
...
<<*source-scigraph-functions>>
deploy-services --config <<build-location-sparc-data()>>/<<file-stem-sparc-data-local()>>.yaml
#+end_src
**** build-deploy
