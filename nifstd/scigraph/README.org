...
function ssh-copy-id-simple () {
    ...
    ssh -t ${SERVER} "${SUDO_OR_SU} '$(typeset -f make-authorized-keys); make-authorized-keys ${SVC_USER} ${TEMP_RANDOM_NAME}.pub'"
...
#+end_src
#+name: &command-ssh-copy-id-remote-bad
#+begin_src bash :eval never
export UHOME="'$(eval echo ~'"${SVC_USER}"')'"
"'echo $UHOME'"
${SUDO_OR_SU}"' \"
if [ ! -d ${UHOME}/.ssh ]; then
    mkdir ${UHOME}/.ssh
...
if [ ! -f ${UHOME}/.ssh/authorized_keys ]; then
    touch ${UHOME}/.ssh/authorized_keys
    chown '"${SVC_USER}:${SVC_USER}"' ${UHOME}/.ssh/authorized_keys
fi'"
cat ${TEMP_RANDOM_NAME}.pub\" >> \
"'${UHOME}/.ssh/authorized_keys'" && \
rm ${TEMP_RANDOM_NAME}.pub
#+end_src
...
Once you have everything set up above run the following
