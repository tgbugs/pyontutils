...
function deploy-graph-localhost () {
    <<&vars-deploy-graph-local>>
    cp ${PATH_ZIP} ${PATH_RUNTIME} || return $?
    pushd ${PATH_RUNTIME} || return $?
    unzip -n ${FILE_NAME_ZIP} || return $?
    unlink ${FOLDER_NAME_RUNTIME} || return $?
    ln -sT ${STAMPED} ${FOLDER_NAME_RUNTIME} || return $?
    popd
}
#+end_src
...
#+begin_src bash :eval never
su - ${SVC_USER} -c '$(typeset -f deploy-graph-localhost); \
deploy-graph-localhost \
--path-zip ${PATH_ZIP} \
--path-runtime ${PATH_RUNTIME} \
--folder-name-runtime ${FOLDER_NAME_RUNTIME}'
#+end_src
