...
hidden pitfalls.
* Using this file
You can either [[https://orgmode.org/worg/org-contrib/babel/how-to-use-Org-Babel-for-R.html][tangle]]
this file in emacs using =C-c C-v t= or you can
[[https://orgmode.org/manual/Extracting-Source-Code.html][export]]
...
**** generic
***** common
Set =build-location-graph= here
#+NAME: build-location-graph
: /tmp/scigraph-build
...
key="$1"
case $key in
    --build-location)    local BUILD_LOCATION="${2}"; shift; shift ;;
...
: ${PATH_ONTOLOGIES:="$(eval echo ${_PATH_ONTOLOGIES})"}
: ${BUILD_LOCATION:="${_BUILD_LOCATION}"}
: ${FOLDER_NAME_GRAPH:="${_FOLDER_NAME_GRAPH}"}
LOAD_GRAPH_PATH="${BUILD_LOCATION}/${FOLDER_NAME_GRAPH}"
...
echo $LOAD_GRAPH_PATH $FOLDER_NAME_GRAPH $STAMPED $ZIPNAME $ZIPPATH
#+end_src

#+name: load-graph
#+header: :noweb yes :eval never
#+header: :shebang "#!/usr/bin/env bash" :tangle-mode (identity #o755)
#+header: :var _PATH_GRAPHLOAD=path-graphload-template()
#+header: :var _PATH_ONTOLOGIES=""
#+header: :var _BUILD_LOCATION=build-location-graph()
#+header: :var _FOLDER_NAME_GRAPH=folder-name-graph()
#+begin_src bash :tangle ./bin/load-graph
<<*vars-load-graph>>
...
***** load sparc data
Set =build-location-graph= here
#+NAME: build-location-sparc-data
...
#+NAME: load-graph-sparc-data
#+header: :noweb yes :eval never :results output
#+header: :shebang "#!/usr/bin/env bash" :tangle-mode (identity #o755)
#+header: :var _PATH_GRAPHLOAD=path-graphload-template()
#+header: :var _PATH_ONTOLOGIES=path-ontologies-sparc-data()
#+header: :var _BUILD_LOCATION=build-location-sparc-data()
#+header: :var _FOLDER_NAME_GRAPH=folder-name-graph-sparc-data()
#+begin_src bash :tangle ./bin/load-graph-sparc-data
<<load-graph>>
#+end_src
...
case $key in
    ...
    --path-graphload)  local PATH_GRAPHLOAD="${2}"; shift; shift ;;
    --path-ontologies) local PATH_ONTOLOGIES="${2}"; shift; shift ;;
    --zip-location)    local ZIP_LOCATION="${2}"; shift; shift ;;
    --git-location)    local GIT_LOCATION="${2}"; shift; shift ;;
...
#+header: :var _PATH_GRAPHLOAD=path-graphload-template()
#+header: :var _BUILD_LOCATION=build-location-graph()
#+header: :var _GIT_REF="master"
#+begin_src bash :noweb yes :tangle ./bin/load-graph-ontload
...
#+name: load-graph-ontload-patch
#+header: :shebang "#!/usr/bin/env bash" :tangle-mode (identity #o755)
#+header: :var _PATH_GRAPHLOAD=path-graphload-template()
#+header: :var _BUILD_LOCATION=build-location-graph()
#+header: :var _GIT_REF="master"
#+begin_src bash :noweb yes :tangle ./bin/load-graph-ontload-patch
<<load-graph-ontload>>
--patch                      ${IFS# do apply patches} \
--patch-config patches.yaml  ${IFS# path to patche files}
#+end_src
***** load nifstd
Set =build-location-nifstd= here
#+name: build-location-nifstd
...
#+name: load-graph-nifstd
#+header: :shebang "#!/usr/bin/env bash" :tangle-mode (identity #o755)
#+header: :var _PATH_GRAPHLOAD=path-graphload-template()
#+header: :var _PATH_ONTOLOGIES=path-ontologies-nifstd()
#+header: :var _BUILD_LOCATION=build-location-nifstd()
#+header: :var _GIT_REF=git-ref-nifstd()
#+BEGIN_SRC bash :noweb yes :tangle ./bin/load-graph-nifstd
<<load-graph-ontload>>
#+end_src
***** load sparc
Set =build-location-sparc= here
#+name: build-location-sparc
: /tmp/scigraph-build/sparc

=path-ontologies-sparc=
...
#+NAME: load-graph-sparc
#+header: :noweb yes :eval never :results output
#+header: :shebang "#!/usr/bin/env bash" :tangle-mode (identity #o755)
#+header: :var _PATH_GRAPHLOAD=path-graphload-template()
#+header: :var _PATH_ONTOLOGIES=path-ontologies-sparc()
#+header: :var _BUILD_LOCATION=build-location-sparc()
#+header: :var _GIT_REF=git-ref-sparc()
#+begin_src bash :tangle ./bin/load-graph-sparc
<<load-graph-ontload>>
#+end_src
**** nifstd-tools only
...
case $key in
    --build-location)    local BUILD_LOCATION="${2}"; shift; shift ;;
    --graph-folder-name) local FOLDER_NAME_GRAPH="${2}"; shift; shift ;;
    --server)            local SERVER="${2}"; shift; shift ;;
    --svc-user)          local SVC_USER="${2}"; shift; shift ;;
    --runtime-location)  local RUNTIME_LOCATION="${2}"; shift; shift ;;
    *) POSITIONAL+=("$1"); shift ;;
esac
done
...
STAMPED="${ZIPNAME//.zip/}"
ZIPPATH="${BUILD_LOCATION}/${ZIPNAME}"
echo $FOLDER_NAME_GRAPH $STAMPED $ZIPNAME $ZIPPATH
#+end_src

#+name: deploy-graph
#+header: :eval never :results output
#+header: :shebang "#!/usr/bin/env bash" :tangle-mode (identity #o755)
#+header: :var _SERVER="localhost"
#+header: :var _RUNTIME_LOCATION="/var/lib/scigraph"
#+header: :var _SVC_USER="scigraph"
#+header: :var _BUILD_LOCATION=build-location-graph()
#+header: :var _FOLDER_NAME_GRAPH=folder-name-graph()
#+begin_src bash :noweb yes :tangle ./bin/deploy-graph
<<*vars-deploy-graph>>
if [ "${SERVER}" = "localhost" ]; then
    ...
    scp ${ZIPPATH} "${scigraph_host}:${RUNTIME_LOCATION}"
    ssh ${scigraph_host} "pushd \"${RUNTIME_LOCATION}\"; unzip \"${ZIPNAME}\""

    ssh ${scigraph_host_admin} "sudo systemctl stop scigraph"

    ssh ${scigraph_host} "unlink \"${RUNTIME_LOCATION}/graph\"
                          ln -sT \"${RUNTIME_LOCATION}/${STAMPED}\" \"${RUNTIME_LOCATION}/graph\""
...
**** sparc
#+begin_src bash
./bin/deploy-graph --server aws-scigraph
#+end_src
**** sparc-data
#+begin_src bash
./bin/deploy-graph --server aws-scigraph-data
#+end_src
** Config
*** Build
**** common
...
#+name: folder-name-graph-services
: graph
...
# FIXME I think we have one too many levels here use noweb for defaults instead?
: ${BUILD_LOCATION:="${_BUILD_LOCATION}"}
: ${FOLDER_NAME_GRAPH:="${_FOLDER_NAME_GRAPH}"}
...
SERVICES_GRAPH_PATH="/var/lib/scigraph/${FOLDER_NAME_GRAPH}"  # FIXME hardcoded assumption
YAML=${FILE_STEM}-${DT}.yaml.raw
YAML_RAW=${FILE_STEM}.yaml
...
#+header: :var _PATH_SERVICES=path-services()
#+header: :var _PATH_CURIES=path-curies()
#+header: :var _FILE_STEM=file-stem-services-output()
#+header: :var _BUILD_LOCATION=build-location-services()
#+header: :var _FOLDER_NAME_GRAPH=folder-name-graph-services()
#+header: :var _SVC_HOST="localhost"
#+header: :var _SVC_PORT=9000
#+begin_src bash :tangle ./bin/build-services
<<*vars-build-services>>
# make raw
cat "${PATH_SERVICES}" > ${YAML_RAW}
cat "${PATH_CURIES}" | sed 's/^/    /' >> ${YAML_RAW}
cat "${PATH_CYPHER_RESOURCES}" >> ${YAML_RAW}
...
#+name: build-services-nifstd
#+header: :noweb yes :eval never :results output
#+header: :shebang "#!/usr/bin/env bash" :tangle-mode (identity #o755)
#+header: :var _PATH_CYPHER_RESOURCES=cypher-resources-nifstd()
#+header: :var _PATH_SERVICES=path-services()
#+header: :var _PATH_CURIES=path-curies()
#+header: :var _BUILD_LOCATION=build-location-services()
#+header: :var _FOLDER_NAME_GRAPH=folder-name-graph-services()
#+header: :var _SVC_HOST="localhost"
#+header: :var _SVC_PORT=9000
#+begin_src bash :tangle ./bin/build-services-nifstd
<<build-services>>
#+end_src
**** sparc
...
#+name: build-services-sparc
#+header: :noweb yes :eval never :results output
#+header: :shebang "#!/usr/bin/env bash" :tangle-mode (identity #o755)
#+header: :var _PATH_CYPHER_RESOURCES=cypher-resources-sparc()
#+header: :var _PATH_SERVICES=path-services()
#+header: :var _PATH_CURIES=path-curies()
#+header: :var _BUILD_LOCATION=build-location-services()
#+header: :var _FOLDER_NAME_GRAPH=folder-name-graph-services()
#+header: :var _SVC_HOST=host-services-sparc()
#+header: :var _SVC_PORT=9000
#+begin_src bash :tangle ./bin/build-services-sparc
<<build-services>>
#+end_src
**** TODO sparc data
#+name: cypher-resources-sparc-data
: ~/git/sparc-curation/resources/scigraph/cypher-resources.yaml
# using the line below without a leading colon works, but it expands
# in emacs, which has a chance of breaking the ability to use the script
# as a user other than the one that tangled the file
# [[file:~/git/sparc-curation/resources/scigraph/cypher-resources.yaml]]

#+name: folder-name-graph-services-sparc-data
: sparc-data

#+name: host-services-sparc-data
: sparc-data.scicrunch.io

#+name: build-services-sparc-data
#+header: :noweb yes :eval never :results output
#+header: :shebang "#!/usr/bin/env bash" :tangle-mode (identity #o755)
#+header: :var _PATH_CYPHER_RESOURCES=cypher-resources-sparc-data()
#+header: :var _PATH_SERVICES=path-services()
#+header: :var _PATH_CURIES=path-curies()
#+header: :var _BUILD_LOCATION=build-location-services()
#+header: :var _FOLDER_NAME_GRAPH=folder-name-graph-services-sparc-data()
#+header: :var _SVC_HOST=host-services-sparc-data()
#+header: :var _SVC_PORT=9000
#+begin_src bash :tangle ./bin/build-services-sparc-data
<<build-services>>
#+end_src
*** Deploy
# TODO
