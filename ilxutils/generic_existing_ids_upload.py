...
#sci = scicrunch_client.scicrunch(base_path='https://scicrunch.org', key='you-key')
#engine_key = 'production-mysql-login' #
...
def append_existing_ids(existing_ids, tid, obj, curie):
    iris = [existing_id['iri'] for existing_id in existing_ids]
    if obj not in iris:
        existing_ids += [{
            'tid':tid,
            'iri':obj,
...
filename = 'OEN-NLXOEN'
new_existing_ids = json.load(open('/Users/tmsincomb/Desktop/dump/Updating_Prefixes/'+filename+'.json'))
...
for i, new_existing_id in enumerate(new_existing_ids[:]):
    ...
    existing_ids = append_existing_ids(
                        existing_ids=existing_ids,
                        tid = _id,
                        obj = new_existing_id['obj'],
                        curie = new_existing_id['curie'],
                    )
