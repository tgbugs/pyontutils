...
from ilxutils.scicrunch_client_fast import scicrunch
import sys
...
def main():
    ...
    labels_to_ids_dict = json.load(
        open('../dump/labels_to_ids_dict_complete_interlex_sql.json', 'r'))
    ...
    new_label_data = [
        label_data for label, label_data in data.items() if
        not labels_to_ids_dict.get(label.lower().strip().replace("'", '&#39;'))
    ][:]
    ...
    for labels_data in new_label_data[:]:
        if labels_data.get('annotations'):
            for annotation in labels_data['annotations']:
                annotations.append({
                    'annotation_tid':
                    annotation['annotation_tid'],
                    'tid':
                    output[labels_data['label'].replace("'", '&#39;')]
                    ['id'],  #need ID to continue; get it from addTerm output
                    'value':
                    annotation['value'],
                })
