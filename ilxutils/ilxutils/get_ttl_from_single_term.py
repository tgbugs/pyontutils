...
class Single_Term_Turtle_Generator():
    ...
    def create_graph(self):
        g = createOntology(
            filename=self.p.stem,
            name='Interlex Total',
            prefixes={
                **{
                    'Testernvs': 'http://whatever.com/'
                },
                **makePrefixes('ILXREPLACE', 'ilx', 'NIFRID', 'NCBIGene', 'NCBITaxon', 'skos', 'owl', 'definition', 'ILX', 'ilxtr', 'oboInOwl')
            },
    ...
    def get_pref_unpref_iris(self, seg_df, terms_ilx):
        ...
        return self.helper_pref_filter(
    ...
    def make_preferred_iris_dict(self):
        ...
        for i, curr_id in enumerate(df.id):
            ...
            pref_iri, unpref_iris = self.get_pref_unpref_iris(
            ...
            for row in seg_df.itertuples():
                ...
                for unpref_iri in unpref_iris:
                    ...
                    if 'neurolex.' in unpref_iri:
                        ...
                        self.g.add_trip(pref_iri, 'ilxtr:existingIds',
                                        unpref_iri)
        ...
        for i, row in df.iterrows():
            ...
            for seg_row in seg_df.itertuples():
                if seg_row.type == 'abbrev':
                    self.g.add_trip(pref_iri, "NIFRID:abbrev",
                                    seg_row.syn_abbrev)
                else:
                    self.g.add_trip(pref_iri, "NIFRID:synonym",
                                    seg_row.syn_abbrev)
    ...
    def superclasses(self):
        ...
        for i, row in enumerate(df.itertuples()):
            g.add_trip(self.pref_dict[row.curr_id], 'rdfs:subClassOf',
                       self.pref_dict[row.superclass_id])
    ...
    def annotation(self):
        ...
        for i, row in df.iterrows():
            ...
            for seg_row in seg_df.itertuples():
                self.g.add_trip(pref_iri, 'ilx:' + row.annotation_ilx,
                                seg_row.value)
    ...
    def relationship(self):
        ...
        for i, row_rel in df.iterrows():
            ...
            subj, pred, obj = self.ilx_to_pref[subj], self.ilx_to_pref[
                pred], self.ilx_to_pref[obj]
...
def main():
    ...
    sttg.relationship()
    sttg.g.g.serialize(
        destination=sttg.args.output, format='turtle')  #g.write() bugged
    print('=== COMPLETE ===')
