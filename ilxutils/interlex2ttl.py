...
def create_graph(filename):
    g = createOntology(filename=filename,
                       ...
                       path='',
                       local_base='/Users/tmsincomb/Desktop/interlexutils/')
    return g
...
def make_preferred_iris_dict(g=None, test_df=None):
    ...
    for i, curr_id in enumerate(df_ids):
        #FIXME looks bad
        if curr_id >= MAX_LIMITER or curr_id <= MIN_LIMITER: continue
    ...
    for i, curr_id in enumerate(df_ids):
        ...
        for row in seg_df.itertuples():
            ...
            if row.type == 'relationship':
                g.add_op(g.qname(pref_iri), row.label) #''.join(row.label.split(' '))) #FIXME why did this need to be compressed?
                #g.add_namespace(row.label.replace(' ' , ''), g.expand(pref_iri))
            elif row.type == 'annotation':
                g.add_ap(g.qname(pref_iri), row.label) #''.join(row.label.split(' '))) #FIXME why did this need to be compressed?
            else:
                g.add_class(pref_iri, label=row.label)
...
def synonym(g, pref_dict):
    ...
    for i, curr_id in enumerate(df_ids):
        ...
        #FIXME, maybe I should just import them all even if they aren't typed
        for row in seg_df.itertuples():
            #if row.type == 'synonyms': g.add_trip(pref_iri, "NIFRID:synonym", row.syn_abbrev)
            if row.type == 'abbrev':
...
if __name__ == '__main__':
    ...
    #g.write()
    #FIXME maybe just run my own serializer from rdflib itself if tom no longer supports make graph
    g.g.serialize(destination='../interlex_test.ttl', format='turtle')
    print('COMPLETE')
