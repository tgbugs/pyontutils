...
build-release () {
    ...
    sdistname=${packagename//-/_}
    pushd dist/
    tar xvzf ${sdistname}-${version}.tar.gz
    pushd ./${sdistname}-${version}/
    python setup.py bdist_wheel $@  # this should NOT be $REST, because we don't call it with --release (among other things)
    mv dist/*.whl ../
    popd  # from ./${sdistname}-${version}/
    rm -r ./${sdistname}-${version}/
    popd  # from dist/
    ...
    if [[ -z ${NO_TEST} ]]; then
        ...
        local CODE VENV_FOLDER PYTHON_EXE
        VENV_FOLDER=$(mktemp -p "${VENV_BASE}" -d venv-release-XXXXX)
        ...
        if [[ -n ${DEBUG} ]]; then
            pip install pudb ipdb  # install both for simplicity
            NOCAP='-s'
        fi
        ...
        if [[ -n ${INTEGRATION_PACKAGES} ]]; then
            ...
            pip install \
                "${maybe_eiu[@]}" \
                ${INTEGRATION_PACKAGES[@]} || { CODE=$?; deactivate; return $CODE; }
        fi
        ...
        [[ -n $CODE && $CODE -ne 0 ]] && { deactivate; return $CODE; }

        pytest ${NOCAP} || local FAILURE=$?
        deactivate
