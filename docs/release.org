...
if [[ ${folder} == *"/"* || -n ${TAG_PREFIX} ]]; then
    ...
    local clone_target=${repo}-${packagename}  # prevent git lock collisions
    folder="${clone_target}/${folder#*/}"
else
    local tag=${version}
    local clone_target=${repo}
fi
...
build-release () {
    ...
    if [[ $(git tag -l ${tag}) ]]; then
        ...
        initpath=$(grep -l __version__ $(ls */__init__.py))
        version=${version}+${gsh}
        tag=${tag}+${gsh}
    ...
    if [ -n ${initpath} ]; then  # apply local version after checkout
        sed -i '/__version__/d' "${initpath}"  # handle bad semantics for find_version
        echo "__version__ = '${version}'" >> "${initpath}"
    fi
