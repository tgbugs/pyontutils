...
def create_graph(filename):
    g = createOntology(filename=filename,
                       ...
                       path='/Users/tmsincomb/Desktop/interlexutils/',
                       local_base='')
    return g
    ...
    for i, curr_id in enumerate(df_ids):
        ...
        for row in seg_df.itertuples():
            ...
            if row.type == 'relationship':
                g.add_op(pref_iri, row.label) #''.join(row.label.split(' '))) #FIXME why did this need to be compressed?
                #g.add_namespace(row.label.replace(' ' , ''), g.expand(pref_iri))
            elif row.type == 'annotation':
                g.add_ap(pref_iri, row.label) #''.join(row.label.split(' '))) #FIXME why did this need to be compressed?
            else:
                g.add_class(pref_iri, label=row.label)
...
def superclasses(g, pref_dict):
    data =  '''
            ...
            '''
    df = pd.read_sql(data, engine_key)
